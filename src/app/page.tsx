...
export default function Dashboard() {
  ...
  function si(st: string) { if (st === "sent") return <span className="material-symbols-rounded inline ml-0.5" style={{ fontSize: 14, opacity: 0.5, color: "var(--bubble-me-text)" }}>check</span>; if (st === "delivered") return <span className="material-symbols-rounded inline ml-0.5" style={{ fontSize: 14, opacity: 0.5, color: "var(--bubble-me-text)" }}>done_all</span>; if (st === "read") return <span className="material-symbols-rounded inline ml-0.5" style={{ fontSize: 14, color: "#53bdeb" }}>done_all</span>; return null; }
                ...
                <div className={`flex ${isMe ? "justify-end" : "justify-start"} ${msg.reaction ? "mb-5" : "mb-[3px]"} group/m`}>
                  <div className="relative max-w-[70%] sm:max-w-[60%]">
                    ...
                    <div className={`relative px-3 py-[7px] ${msg.message_type === "sticker" ? "" : replied ? "rounded-b-2xl" : "rounded-2xl"}`} style={msg.message_type === "sticker" ? {} : { background: isMe ? "var(--bubble-me)" : "var(--bubble-them)", color: isMe ? "var(--bubble-me-text)" : "var(--bubble-them-text)", boxShadow: "var(--shadow-sm)", ...(isMe && !replied ? { borderTopRightRadius: "6px" } : !isMe && !replied ? { borderTopLeftRadius: "6px" } : {}) }}>
                      ...
                      {media(msg)}
                      <div className="flex items-center justify-end gap-0.5 mt-0.5">{msg.is_starred && <span className="material-symbols-rounded" style={{ fontSize: 12, color: "#eab308", fontVariationSettings: "'FILL' 1" }}>star</span>}<span className="text-[10px]" style={{ opacity: 0.5 }}>{mt(msg.created_at)}</span>{isMe && si(msg.status || "sent")}</div>
                    </div>
